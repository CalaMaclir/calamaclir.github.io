...
function resetUI() {
    ...
    const exportUrlBtn = document.getElementById('exportUrlPubkeyBtn');
    if (exportUrlBtn) exportUrlBtn.style.display = 'none';
    currentUrlPubkeyXml = null;
    ...
    if (location.hash.startsWith("#pubkey=")) {
        try {
            ...
            if (expectedFp && pubKey.fingerprint !== expectedFp) {
                ...
                currentUrlPubkeyXml = xml;
                const exportUrlBtn = document.getElementById('exportUrlPubkeyBtn');
                if (exportUrlBtn) exportUrlBtn.style.display = 'inline-block';
...
if (exportUrlBtn) {
    exportUrlBtn.addEventListener('click', () => {
        if (!currentUrlPubkeyXml) return;
        const blob = new Blob([currentUrlPubkeyXml], { type: "application/xml" });
        const url = URL.createObjectURL(blob);
        const a = document.createElement("a");
        a.href = url;
        a.download = "URL_Imported_Key.pubkey";
        document.body.appendChild(a);
        a.click();
        document.body.removeChild(a);
    });
